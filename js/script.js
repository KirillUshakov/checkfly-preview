...
let fixedElementsQuery = ['body', '#header-main'];
let fixedElements = [];
...
testWebP(function (support) {
  ...
  const headerMobileBP = 992;
  const farGap = window.innerHeight * 1.5;
  const classNames = {
    active: '_active',
    close: '_close',
    fixed: '_fixed',
  ...
  const scrollHandler = (e) => {
    ...
    if (scrolled <= 0) {
      header.classList.add(classNames.clipped);
    } else {
      header.classList.remove(classNames.clipped);
    }

    if (scrolled >= farGap) {
      header.classList.add(classNames.far);
    } else {
      header.classList.remove(classNames.far);
    }
  }
  const setHandlers = () => {
    ...
    if (window.innerWidth > headerMobileBP) {
      burgerContainer.addEventListener('mouseenter', () => setBurgerClasses());
      burgerContainer.addEventListener('mouseleave', () => setBurgerClasses(false));
    } else {
      burgerBtn.addEventListener('click', burgerBtnClickHandler);
    }
  }
  const afterHeaderPlaceholderInit = () => {
    header.classList.add(classNames.fixed);

    if (!header.classList.contains('header--inner')) return;

    // Move placeholder to section--first
    const placeholder = document.getElementById('header-placeholder');
    const firstSection = document.querySelector('section.section--first');
    firstSection.insertBefore(placeholder.cloneNode(true), firstSection.firstChild);
    placeholder.remove()
  }
  const init = () => {
    setHandlers();
...
initHeaderMenu();
  ...
  // Sliders
  const defaultSlider = () => {
    initSlider('.slider', (sliderWrapper) => {
...
popupTrigers.forEach(trigger => {
  ...
  trigger.onclick = async () => {
    ...
    openPopup(popupId, trigger.closest('.popup') ? false : true, popupTitle);
  };
});
...
document.addEventListener('click', (e) => {
  ...
  const generateSelectHtml = (dSelect) => {
    ...
    const getSelectorList = (optionsWithLabels = []) => {
      const getOptionTpl = (optionObj = {}) => /*html*/`
        <button type="button" data-label="${optionObj.label}" data-value="${optionObj.value}" class="selector__option ${optionObj.disabled ? 'disabled' : ''} ${optionObj.selected ? 'active' : ''} ${optionObj.value ? '' : 'hidden'}">
  ...
  const init = (wrapper) => {
    ...
    function filterOptionList (inputVal = input.value.toLowerCase()) {
      options.forEach(option => {
        ...
        if (!inputVal && !isEmptyOption) {
          option.classList.remove('hidden-option');
        } else {
          if (optionValue.indexOf(inputVal) != -1 && !isEmptyOption) {
            option.classList.remove('hidden-option');
          } else {
    ...
    function optionHandler (option, e) {
      e.stopPropagation();
  ...
  dSelects.forEach(el => generateSelectHtml(el));
}
initCustomSelectors();
...
initInputMasks();
  ...
  const init = (wrap) => {
    const datepicker = new Datepicker(wrap, {
      toValue: (str) => {
        if (!str) return '';

        const date = new Date(str);
        const returnDate = date.toLocaleDateString('ru-RU', { dateStyle: 'long' });
        return returnDate.replace(' г.', '');
      },
    });
  }

  dateInputs.forEach(input => init(input));
}
initDatepicker();
  ...
  const repeatFields = form.querySelectorAll('.form__repeat-fields');
  ...
  const formInputsUpdateHandler = () => {
    const passengerCount = form.querySelectorAll('.form__repeat-field[data-name="passenger"]');

    if (passengerCount) {
      updatePassengerCount(passengerCount.length);
    }
  }
  // -- Repeat Fields
  const initRepeatFields = (wrapper) => {
    const repeatField = wrapper.querySelector('.form__repeat-field');
    const repeatBtn = wrapper.querySelector('.form__repeat-btn');
    let repeatCount = 0;

    if (!repeatField || !repeatBtn) return;

    const deleteRepeatField = (field) => {
      field.remove();
      formInputsUpdateHandler();
    }
    const normalizeNewField = (field) => {
      const deleteBtn = field.querySelector('.button--delete');
      const inputs = field.querySelectorAll('input');

      inputs.forEach(input => {
        input.name = input.name.match(/[^-]+/gm) + '-' + repeatCount;
      });

      deleteBtn.addEventListener('click', (e) => {
        e.preventDefault();
        deleteRepeatField(field);
      })
    }
    const getNewField = () => {
      const repeatFieldClass = 'form__repeat-field--dynamic';
      const deleteBtnTpl = /*html*/`
        <button type="button" class="btn button button--icon button--delete button--light">
          ${getIcon('trash')}
        </button>
      `;
      const newField = repeatField.cloneNode(true);

      newField.classList.add(repeatFieldClass);
      newField.innerHTML += deleteBtnTpl;

      normalizeNewField(newField);

      return newField;
    }
    const addNewField = () => {
      wrapper.insertBefore(getNewField(), repeatBtn);
      repeatCount++;
      formInputsUpdateHandler();
    }
    const addHandlersToDefault = () => {
      const existRepeatFields = wrapper.querySelectorAll('.form__repeat-field--dynamic');

      existRepeatFields.forEach(el => {
        normalizeNewField(el);
        repeatCount++;
      });
    }

    addHandlersToDefault();
  ...
  formInputsUpdateHandler();
  repeatFields.forEach(wrapper => initRepeatFields(wrapper));
}
